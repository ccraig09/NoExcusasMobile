import cardioCategory from "../models/cardioCategory";

const CARDIOCATEGORY = [
  new cardioCategory(
    "c1",
    "Cardio",
    "https://i.imgur.com/TxWfEGv.png",
    "NIVEL 1",
    "25m",
    "Principiante",
    "../assets/icon-noexlogo.png",
    29.99,
    "Class about Cardio to improve stamina",
    "https://www.googleapis.com/drive/v3/files/1eqhEWxabUTSuKJ_kGYuT-wlWyPkv80wk?alt=media&key=AIzaSyCAOrqiYxKUcbPhLJuGknCBOGDnw_S2DKw"
  ),
  new cardioCategory(
    "c2",
    "Cardio 2",
    "https://i.imgur.com/VOPt2Us.png",
    "NIVEL 2",
    "30m",
    "Medio",
    "../assets/icon-noexlogo.png",
    99.99,
    "Class about Strength to improve stamina",
    "https://drive.google.com/uc?export=download&id=14EveartXxW2h7yliQlTWBTJZpb8D09dW"
  ),
  new cardioCategory(
    "c3",
    "Cardio 3",
    "https://i.imgur.com/ymOClmI.jpg",
    "NIVEL 3",
    "30m",
    "Avanzado",
    "../assets/icon-noexlogo.png",
    8.99,
    "Class about Mixed to improve stamina",
    "https://drive.google.com/uc?export=download&id=1vs4ZFJ-y_oiAhGybG0P2e2cgiQcfGeCC"
  ),
];

export default CARDIOCATEGORY;
